...
predicate existsVotesForCandidate(V : Votes, c : Candidate)
...
method {:vcs_split_on_every_assert} TransferVotes(V : Votes, F : FactorList, c : Candidate, q : real, m : real) returns (A0 : Tuple)
    /*
    ...
    Input:
        V (Votes) : Sequence of the current ballots
...
lemma IndicesBelowF(V : Votes, c : Candidate, F : FactorList, m : real)
