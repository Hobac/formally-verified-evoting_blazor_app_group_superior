...
lemma RankingLemma(R : map<Candidate, real>, V : Votes, F : FactorList,m : Value, C : Candidates)
...
lemma removingLemma(V : Votes, A0 : Votes, C : Candidates, c : Candidate)
...
lemma winnersUniqueness(V : Votes, W : Candidates,  C : Candidates ,c : Candidate)
...
lemma containmentLemma(V : Votes)
    ensures forall i :: 0 <= i < |V| ==> V[i] in V
{
}

lemma /*{:vcs_split_on_every_assert}*/ votesLemma(V : Votes, V0 : Votes, V1 : Votes, F0 : FactorList, F1 : FactorList, C : Candidates, C0 : Candidates, c : Candidate)
    requires forall i,c :: 0 <= i < |V| && c in V[i] ==> multiset(V[i])[c] <= 1
    requires forall f :: f in F0 ==> f >= 0.0
    requires |V0| == |F0|
    requires forall c :: c in C ==> multiset(C)[c] == 1
    requires V0 == removeCandidate(V,c)
    requires V1 == removeEmptyBallots(V0, F0).0
    requires F1 == removeEmptyBallots(V0, F0).1
    requires C0 == removeSeqCandidates(C, c)
    requires forall v,c :: v in V && c in v ==> c in C
    ensures forall v,c :: v in V1 && c in v ==> c in C0
    ensures forall v :: v in V1 ==> |v| > 0
    ensures forall c :: c in C0 ==> multiset(C0)[c] == 1
    ensures forall v,c :: v in V1 && c in v ==> multiset(v)[c] == 1
    ensures |V1| == |F1|
    ensures forall f :: f in F1 ==> f >= 0.0
{
    assert V0 == removeCandidate(V, c);
    assert forall i, c :: 0 <= i < |V0| && c in V0[i] ==> c in V[i];
    containmentLemma(V);
    containmentLemma(V1);
    containmentLemma(V0);
    assert forall v,c :: v in V0 && c in v ==> c in C;
    assert forall i :: 0 <= i < |V1| ==> V1[i] in V0;
    assert forall v,c :: v in V1 && c in v ==> c in C;
    assert forall v,a :: v in V1 && a in v ==> a != c;
    assert forall f :: f in F1 ==> f in F0;
    assert V1 == removeEmptyBallots(V0, F0).0;
        
}

lemma concatenationLemma(stateSet : seq<Triple>, V0 :Votes, F0 : FactorList, C0 : Candidates, W : Candidates, V : Votes, s : seats)
...
method {:vcs_split_on_every_assert}/*{:timeLimit 20}*/ SingleTransferableVote(V : Votes, C : Candidates, s : seats) returns (W : Candidates, Rest : Candidates, stateSet : seq<Triple>)
...
/*
    ...
    Input:
        V (Votes) : Sequence of all registered votes
