...
function removeCandidate(V : Votes, c : Candidate) : (A0 : Votes)
...
lemma incrementalSeq5(s : Votes, i : nat)
    requires -1 <= i < |s|
    ensures s[i..] == [s[i]] + s[i+1..]
{
    assert s[i..] == [s[i]] + s[i+1..];
...
lemma RemoveVotes(V : Votes, c : Candidate)
    requires forall i :: 0 <= i < |V| ==> multiset(V[i])[c] <= 1
    requires forall i,c :: 0 <= i < |V| && c in V[i] ==> multiset(V[i])[c] <= 1 
...
method RemoveCandidate(V : Votes, c : Candidate) returns (A0 : Votes)
    requires forall i :: 0 <= i < |V| ==> multiset(V[i])[c] <= 1
...
lemma incrementalSeq7(s1 : PreferenceList, s2 : Votes)
    ensures ([s1] + s2)[1..] == s2
{
    assert ([s1] + s2)[1..] == s2;
}
...
lemma emptyBallots(V : Votes, F : FactorList)
    requires |V| == |F| > 0
...
function removeEmptyBallots(V : Votes, F : FactorList) : (A0 : Tuple)
    requires |V| == |F|
    requires forall i,j :: 0 <= i < |V| && 0 <= j < |V[i]| ==> multiset(V[i])[V[i][j]] <= 1
    ensures |A0.0| == |A0.1|
...
/**
    ...
    Input: 
        V (Votes) : A sequence of all registered preference lists
        F (FactorLis) : A sequence of factors corresponding to the preference list at the same index of V
...
method RemoveEmptyBallots(V : Votes, F : FactorList) returns (A0 : Tuple)
    requires |V| == |F|
    requires forall i,j :: 0 <= i < |V| && 0 <= j < |V[i]| ==> multiset(V[i])[V[i][j]] <= 1
    ensures |A0.0| == |A0.1|
    ensures forall v :: v in A0.0 ==> |v| > 0
